    ...
    <>
          ...
          {["S", "M", "L", "XL", "XXL"].map((size) => (
            <button
              ...
              className={`w-12 h-12 border-2 font-bold text-sm transition
                ${selectedSize === size ? "border-[#6A0DAD] bg-[#6A0DAD] text-white" : "border-gray-300 bg-white text-gray-700 hover:border-[#6A0DAD]"}`}>
              {size}
            </button>
          ))}
        </div>
      ...
      <div>
        ...
        <div className="flex items-center border-2 border-gray-300 w-max">
          <button onClick={handleDecrement} className="px-5 py-3 text-xl hover:bg-gray-100">
            -
          </button>
          <span className="px-8 py-3 font-bold text-lg border-x-2 border-gray-300">{quantity}</span>
          <button onClick={handleIncrement} className="px-5 py-3 text-xl hover:bg-gray-100">
            +
          </button>
        </div>
      </div>
    ...
    <main className="py-6 px-4 sm:px-6 lg:px-[6%] max-w-7xl mx-auto font-nunito">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 lg:gap-8">
        ...
        <div className="flex flex-col">
          ...
          <p className="text-xl  text-[#6A0DAD] mb-2">${product.price.toFixed(2)}</p>
          ...
          <div className="mb-6">
            ...
            <div className="flex gap-6 border-b border-gray-200 mb-4">
              <button onClick={() => setActiveTab("details")} className={`pb-3 text-lg font-bold relative ${activeTab === "details" ? "text-gray-900" : "text-gray-500"}`}>
                Details
                {activeTab === "details" && <span className="absolute text-[12px]  bottom-0 left-0 w-full h-1 bg-[#6A0DAD]"></span>}
              </button>
              <button onClick={() => setActiveTab("description")} className={`pb-3 text-lg font-bold relative ${activeTab === "description" ? "text-gray-900" : "text-gray-500"}`}>
                Description
          ...
          <div className="flex flex-col gap-4">
            <button onClick={handleAddToCart} className="w-full bg-[#6A0DAD] hover:bg-[#5a0ca0] text-white text-[12px]  font-bold text-lg py-2 transition">
              Add to Cart
            </button>
            <Link to="/category/men" className="w-full text-center text-[12px]  border-2 border-[#6A0DAD] hover:border-[#5a0ca0] text-[#6A0DAD] font-bold text-lg py-2 transition">
              Continue Shopping
            </Link>
          </div>
        </div>
